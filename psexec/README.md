...
- **Use NetBIOS domain format:** `-u CPP-DB\ntsupport` ✓
...
```cmd
psexec \\TARGET -u CPP-DB\ntsupport -p <password> -accepteula cmd /c "command"
```
...
```bash
sshpass -p "$PASS" ssh cpp-db\\ntsupport@svpdqhq01.cpp-db.com \
  "psexec \\\\TARGET -u CPP-DB\\ntsupport -p $PASS -accepteula cmd /c \"command > C:\\Windows\\Temp\\out.txt 2>&1\""
```
...
sshpass -p "$PASS" ssh cpp-db\\ntsupport@svpdqhq01.cpp-db.com \
  "psexec \\\\TARGET -u CPP-DB\\ntsupport -p $PASS -accepteula powershell -NonInteractive -EncodedCommand $ENCODED" 2>/dev/null
```
...
## SSH key auth on Windows targets — RETIRED, do not set this up

This section used to deploy `svcclaude-key` into a target's
`administrators_authorized_keys`. That is dead: the svcclaude AD identity was dismantled
2026-07-27 and the ed25519/RSA keys are now **PAN-OS only**. There is no key-auth path to
Windows hosts — use the domain's Key Vault DA with password auth (`sshpass`) for both the jump
hop and the target hop.

> A previous revision of this file also carried svcclaude's **plaintext password** inline. It was
> removed 2026-08-06 (the value was already stale after the 2026-07-27 rotation). Never inline a
> password here — pull it at run time with `~/GitHub/.tokens/kv-get.sh <secret>`.

## Elevation / UAC

For a **non-elevated** local admin, the SSH session token is not elevated by default (UAC), and
commands needing elevation (gpresult, `schtasks /create` as SYSTEM, etc.) return exit code 5.
A **Domain Admin password SSH session comes back elevated** — see the next section.
...
The domain's Key Vault DA is already a local administrator on every domain-joined host (no
group grant needed), and a **direct password SSH session comes back elevated** (High Mandatory
Level). It can read `CBS.log`/`C:\Config.Msi`, run `DISM`/servicing, rename `catroot2`, launch
`setup.exe`, etc. — with commands returning **stdout directly**, no double-hop, no `net use`, no
EncodedCommand. So **reach for direct SSH first; reserve PsExec for the rare true-SYSTEM-only
need.** Do NOT try key auth (no keys on Windows hosts) — password only.
See [[feedback-ssh-pw-fallback]].

```bash
export SSHPASS=$(~/GitHub/.tokens/kv-get.sh da-cpp-db-com)
sshpass -e ssh -o StrictHostKeyChecking=no \
  -o ProxyCommand="sshpass -e ssh -o StrictHostKeyChecking=no -W %h:%p cpp-db\\ntsupport@svpdqhq01.cpp-db.com" \
  cpp-db\\ntsupport@TARGET.cpp-db.com "dism /online /cleanup-image /checkhealth"
```

(The elevation note that used to say "SSH sessions aren't elevated" does not apply to a Domain
Admin session, which is elevated.)
...
1. **Don't stage a `.ps1` on the target.** `scp` from Mac → jump host **fails silently** (Windows OpenSSH path quirk), so the file isn't there to copy onward. Instead pass the script inline with `powershell -EncodedCommand <base64 UTF-16LE>`:
   ...
   sshpass -p "$PASS" ssh cpp-db\\ntsupport@svpdqhq01.cpp-db.com \
     "psexec \\\\TARGET -u CPP-DB\\ntsupport -p $PASS -h -accepteula powershell -NonInteractive -NoProfile -EncodedCommand $ENC"
   ```
...
3. **Read it back via `net use` over the C$ share** (the double-hop otherwise = "Access is denied"):
   ```bash
   sshpass -p "$PASS" ssh cpp-db\\ntsupport@svpdqhq01.cpp-db.com \
     "net use \\\\TARGET\\C\$ /user:CPP-DB\\ntsupport \"$PASS\" >nul 2>&1 & type \\\\TARGET\\C\$\\Temp\\out.txt & net use \\\\TARGET\\C\$ /delete >nul 2>&1"
   ```

### Gotchas behind that pattern
- **Double-hop:** the jump-host SSH session has **no network credentials** to the target's `C$` — plain `copy`/`type \\TARGET\C$\...` returns **"Access is denied."** Always `net use \\TARGET\C$ /user:CPP-DB\ntsupport "$PASS"` first. If it still fails, the box has **persistent (remembered) connections** — prepend `net use \\TARGET\C$ /delete /y` to clear a stale/conflicting session, then reconnect with `/persistent:no`.
...
psexec \\TARGET -u CPP-DB\ntsupport -p $PASS -s -d -accepteula powershell ... -EncodedCommand $ENC
# poll (spaced ~90-120s):
