...
**PRTG rule:** Collections marked "Yes" suppress PRTG (pause root node `id=0`) for the duration of the run. `DEV/QA/VDI` is exempt — those machines don't carry production monitoring worth silencing, and pausing the root node for them would suppress the whole tree for no benefit. Ad-hoc machine lists follow the same default as their collection; when in doubt, suppress.
...
| Collection | CollectionId | F5 Required | F5 Config Key | PRTG Suppression |
|---|---|---|---|---|
| `Backup` | 3238 | No | — | Yes |
| `DEV/QA/VDI` | 3233 | No | — | **No** |
| `Domain Controllers - Group 1` | 3239 | No | — | Yes |
| `Domain Controllers - Group 2` | 3528 | No | — | Yes |
| `Web Staggered - Group 1` | 3246 | Yes | `web-staggered-first-group` | Yes |
| `Web Staggered - Group 2` | 3545 | Yes | `web-staggered-second-group` | Yes |
| `PROD` | 3236 | Yes | `prod-downtime` | Yes |
